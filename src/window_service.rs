...
/// Find a window by name using exact (full) matching.
...
pub fn find_by_name<'a>(windows: &'a [WindowInfo], name: &str) -> Result<&'a WindowInfo, AppError> {
    windows
        .iter()
        .find(|w| w.title == name || w.app_name == name)
...
mod tests {
    ...
    // WIN-02: exact name matching (case-sensitive)
    #[test]
    fn test_find_by_name_exact_match() {
        let windows = sample_windows();
        let result = find_by_name(&windows, "Firefox - Wikipedia");
        assert!(result.is_ok());
        assert_eq!(result.unwrap().title, "Firefox - Wikipedia");
    }

    // WIN-02: case-sensitive matching - lowercase should not match
    #[test]
    fn test_find_by_name_case_sensitive_no_match() {
        let windows = sample_windows();
        let result = find_by_name(&windows, "firefox - wikipedia");
        assert!(result.is_err());
    }

    // WIN-02: partial substring should NOT match (must be full match)
    #[test]
    fn test_find_by_name_partial_no_match() {
        let windows = sample_windows();
        let result = find_by_name(&windows, "Wiki");
        assert!(result.is_err());
    }

    // WIN-02: exact app_name matching
    #[test]
    fn test_find_by_name_app_name_exact_match() {
        let windows = sample_windows();
        let result = find_by_name(&windows, "iTerm2");
        assert!(result.is_ok());
